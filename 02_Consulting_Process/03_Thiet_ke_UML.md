...
5. **Blueprint ERD (Base Architecture):** Vẽ sơ đồ thực thể liên kết (ERD) dùng cú pháp PlantUML chặt chẽ. Đánh dấu Type (Single Select, N:N, AutoNumber) -> Sẵn sàng cho Giai đoạn 5.

---

## Chuẩn bị chạy Build Phase (Phase 5)
Kỹ sư dùng sơ đồ ERD từ Giai đoạn 3 làm Input cho công cụ Auto-Build (`lark-mcp`). Các yêu cầu cho ERD:
- Chú thích loại trường ngay trong sơ đồ: `(Text)`, `(Single Select)`, `(Link)`, `(User)`.
- Xác định rõ quan hệ `1:1`, `1:N` để bot Lark hiểu và thiết lập `multiple: true/false` trong Base.
- Primary Key (Trường đầu tiên) bắt buộc.
